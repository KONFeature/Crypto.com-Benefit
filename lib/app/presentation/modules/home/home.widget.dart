...
mixin HomeWidget {
  ...
  /// Method called when the user click on a tab
  void onTabSelected(TabPages tabPage);

  /// Check if the specified tab is the current tab
  bool isCurrentTab(TabPages tabPage);
  ...
  FloatingActionButton fab(BuildContext context) =>
      FloatingActionButton.extended(
          ...
          onPressed: () {
            onTabSelected(TabPages.dashboard);
          });
  ...
  Widget _buttonForPage(BuildContext context, TabPages page) => MaterialButton(
      ...
      onPressed: () {
        // Send the event to the page
        onTabSelected(page);
      },
      child: AnimatedOpacity(
        opacity: isCurrentTab(page) ? 1.0 : 0.3,
